...
template <class...> struct always : true_type {};
template <class...> struct never : false_type {};
namespace detail {
...
#endif
template <class T, class U> struct is_empty_base : T {
    U _;
};
template <class T>
struct is_empty
...
template <class R, class... TArgs> struct function_traits<R (*)(TArgs...) noexcept> {
    using args = type_list<TArgs...>;
};
template <class R, class... TArgs> struct function_traits<R(TArgs...) noexcept> {
    using args = type_list<TArgs...>;
};
template <class R, class T, class... TArgs> struct function_traits<R (T::*)(TArgs...) noexcept> {
    using args = type_list<TArgs...>;
};
...
#endif
template <int N> using make_index_sequence = typename make_index_sequence_impl<N>::type;
...
template <class... Ts> struct join<type_list<Ts...>> : type_list<Ts...> {};
...
template <class... Rs> struct unique_impl<type<Rs...>> : type_list<Rs...> {};
template <class... Ts> struct unique : unique_impl<type<>, Ts...> {};
...
template <class T> struct is_unique<T> : true_type {};
template <class T, class... Rs, class... Ts>
struct is_unique<type<Rs...>, T, Ts...>
    : conditional_t<is_base_of<type<T>, inherit<type<Rs>...>>::value,
          false_type,
          is_unique<type<Rs..., T>, Ts...>> {};
template <class... Ts> using is_unique_t = is_unique<type<>, Ts...>;
template <template <class...> class, class> struct apply;
template <template <class...> class T, template <class...> class U, class... Ts>
struct apply<T, U<Ts...>> {
    using type = T<Ts...>;
};
template <template <class...> class T, class D> using apply_t = typename apply<T, D>::type;
template <int, class T> struct tuple_type {
...
template <> struct tuple_impl<index_sequence<0>> {
    __BOOST_SML_ZERO_SIZE_ARRAY(byte);
};
template <class... Ts> using tuple = tuple_impl<make_index_sequence<sizeof...(Ts)>, Ts...>;
template <int N, class T> constexpr T &get_by_id(tuple_type<N, T> *object) {
...
template <class T, class = void> struct pool_type_impl : pool_type_base {
    constexpr explicit pool_type_impl(T object) : value{object} {}
    template <class TObject> constexpr pool_type_impl(init i, TObject object) : value{i, object} {}
    T value{};
};
#if defined(BOOST_SML_CREATE_DEFAULT_CONSTRUCTIBLE_DEPS)
template <class T>
struct pool_type_impl<T &,
    aux::enable_if_t<aux::is_constructible<T>::value && aux::is_constructible<T, const T &>::value>>
    : pool_type_base {
...
#endif
template <class T> struct pool_type : pool_type_impl<T> {
    using pool_type_impl<T>::pool_type_impl;
};
template <class T> struct missing_ctor_parameter {
    static constexpr auto value = false;
    constexpr auto        operator()() const { return T{}(); }
    template <class U,
        __BOOST_SML_REQUIRES(
            !aux::is_base_of<pool_type_base, U>::value && aux::is_constructible<U>::value)>
...
template <class T> constexpr missing_ctor_parameter<T> try_get(...) { return {}; }
template <class T> constexpr T        try_get(const pool_type<T> *object) { return object->value; }
template <class T> constexpr const T &try_get(const pool_type<const T &> *object) {
    return object->value;
}
template <class T> constexpr T &try_get(const pool_type<T &> *object) { return object->value; }
template <class T> constexpr const T *try_get(const pool_type<const T *> *object) {
...
template <class T> constexpr T *try_get(const pool_type<T *> *object) { return object->value; }
template <class T, class TPool> constexpr bool would_instantiate_missing_ctor_parameter() {
    return is_same<missing_ctor_parameter<T>, decltype(try_get<T>(aux::declval<TPool>()))>::value;
}
template <class T, class TPool> constexpr T &get(TPool &p) {
    return static_cast<pool_type<T> &>(p).value;
}
template <class T, class TPool> constexpr const T &cget(const TPool &p) {
    return static_cast<const pool_type<T> &>(p).value;
}
template <class T, class TPool> T *get(TPool *p) { return static_cast<pool_type<T> &>(p).value; }
template <class T, class TPool> const T *cget(const TPool *p) {
    return static_cast<const pool_type<T> &>(p).value;
}
template <class... Ts> struct pool : pool_type<Ts>... {
...
template <> struct pool<> {
    ...
    pool()                  = default;
    template <class... Ts> constexpr explicit pool(Ts &&...) {}
    __BOOST_SML_ZERO_SIZE_ARRAY(byte);
};
template <int, class> struct type_id_type {};
template <class, class...> struct type_id_impl;
template <int... Ns, class... Ts>
struct type_id_impl<index_sequence<Ns...>, Ts...> : type_id_type<Ns, Ts>... {};
template <class... Ts> struct type_id : type_id_impl<make_index_sequence<sizeof...(Ts)>, Ts...> {};
template <class R, class T, int N> constexpr R get_id(type_id_type<N, T> *) {
    return static_cast<R>(N);
}
template <template <class...> class, class T> struct is : false_type {};
template <template <class...> class T, class... Ts> struct is<T, T<Ts...>> : true_type {};
template <class> struct size;
template <template <class...> class T, class... Ts> struct size<T<Ts...>> {
    static constexpr auto value = sizeof...(Ts);
};
template <int... Ts> constexpr int max_element() {
    int max = 0;
    (void) swallow{0, (Ts > max ? max = Ts : max)...};
    return max;
}
template <class TExpr, class = void> struct zero_wrapper : TExpr {
    using type = TExpr;
...
template <class R, class... TArgs, class T> struct zero_wrapper<R (*)(TArgs...), T> {
    using type = R (*)(TArgs...);
...
template <class R, class... TArgs, class T> struct zero_wrapper<R (*)(TArgs...) noexcept, T> {
...
#endif
template <class, class> struct zero_wrapper_impl;
template <class TExpr, class... TArgs> struct zero_wrapper_impl<TExpr, type_list<TArgs...>> {
...
struct zero_wrapper<TExpr, void_t<decltype(+declval<TExpr>())>>
    ...
    using type = TExpr;
    template <class... Ts> constexpr zero_wrapper(Ts &&...) {}
    const TExpr &get() const { return reinterpret_cast<const TExpr &>(*this); }
};
namespace detail {
    template <class, int N, int... Ns> auto get_type_name(const char *ptr, index_sequence<Ns...>) {
...
template <class T> const char *get_type_name() {
    ...
    return detail::get_type_name<T, 72>(
        __PRETTY_FUNCTION__, make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 72 - 2>{});
#endif
}
#if defined(__cpp_nontype_template_parameter_class) ||                                             \
    defined(__cpp_nontype_template_args) && __cpp_nontype_template_args >= 201911L
template <auto N> struct fixed_string {
    static constexpr auto size = N;
    char                  data[N + 1]{};
    constexpr fixed_string(char const *str) {
        for (decltype(N) i = 0; i < N; ++i) { data[i] = str[i]; }
    }
};
template <auto N> fixed_string(char const (&)[N]) -> fixed_string<N - 1>;
#endif
template <class T, T...> struct string;
template <char... Chrs> struct string<char, Chrs...> {
    using type = string;
    static auto c_str() {
...
template <class T> struct string<T> {
    using type = T;
    constexpr static auto                                    c_str() { return c_str_impl((T *) 0); }
    template <class U> constexpr static decltype(U::c_str()) c_str_impl(U *) { return U::c_str(); }
    constexpr static auto c_str_impl(...) { return get_type_name<T>(); }
};
} // namespace aux
template <class T> constexpr auto wrap(T callback) { return aux::zero_wrapper<T, T>{callback}; }
namespace back {
namespace policies {
    struct defer_queue_policy__ {};
    template <template <class...> class T>
    struct defer_queue : aux::pair<back::policies::defer_queue_policy__, defer_queue<T>> {
        template <class U> using rebind = T<U>;
        using flag                      = bool;
    };
} // namespace policies
} // namespace back
namespace back {
template <class... Ts> class queue_event {
    ...
    template <class T> constexpr static void dtor_impl(aux::byte *data) {
        (void) data;
  ...
  public:
    ...
    template <class T> constexpr queue_event(T object) {
...
template <class TEvent> class queue_event_call {
...
template <class... TEvents> struct queue_handler : queue_event_call<TEvents>... {
    ...
    template <class TEvent> constexpr void operator()(const TEvent &event) {
...
template <class... TEvents> struct deque_handler : queue_event_call<TEvents>... {
...
template <class T, class TEvent = T> struct on_entry : internal_event, entry_exit {
    constexpr static auto c_str() { return "on_entry"; }
    constexpr explicit on_entry(const TEvent &event = {}) : event_(event) {}
    const TEvent &event_;
};
template <class T, class TEvent = T> struct on_exit : internal_event, entry_exit {
    constexpr static auto c_str() { return "on_exit"; }
    constexpr explicit on_exit(const TEvent &event = {}) : event_(event) {}
    const TEvent &event_;
};
template <class T, class TException = T> struct exception : internal_event {
...
template <class T, class TEvent = T> struct unexpected_event : internal_event, unexpected {
    constexpr explicit unexpected_event(const TEvent &event = {}) : event_(event) {}
    const TEvent &event_;
};
template <class TEvent> struct event_type {
    using event_t   = TEvent;
    using generic_t = TEvent;
    using mapped_t  = void;
};
template <class TEvent> struct event_type<exception<TEvent>> {
    using event_t   = TEvent;
    using generic_t = exception<TEvent>;
    using mapped_t  = void;
};
template <class TEvent, class T> struct event_type<unexpected_event<T, TEvent>> {
    using event_t   = TEvent;
    using generic_t = unexpected_event<T>;
    using mapped_t  = void;
};
template <class TEvent, class T> struct event_type<on_entry<T, TEvent>> {
    using event_t   = TEvent;
    using generic_t = on_entry<T>;
    using mapped_t  = on_entry<T, TEvent>;
};
template <class TEvent, class T> struct event_type<on_exit<T, TEvent>> {
    using event_t   = TEvent;
    using generic_t = on_exit<T>;
    using mapped_t  = on_exit<T, TEvent>;
};
template <class TEvent> using get_event_t   = typename event_type<TEvent>::event_t;
template <class TEvent> using get_generic_t = typename event_type<TEvent>::generic_t;
template <class TEvent> using get_mapped_t  = typename event_type<TEvent>::mapped_t;
template <class... TEvents> struct process : queue_handler<TEvents...> {
    using queue_handler<TEvents...>::queue_handler;
};
template <class... TEvents> struct defer : deque_handler<TEvents...> {
...
template <class TSrc, class TDst, class TEvent> struct get_all_events_impl<sm<TSrc>, TDst, TEvent> {
    using type = aux::join_t<get_event<TEvent>, typename sm<TSrc>::events>;
};
template <class TSrc, class TDst, class TEvent> struct get_all_events_impl<TSrc, sm<TDst>, TEvent> {
...
template <class, class TEvent> struct get_sub_internal_events_impl {
    using type = aux::conditional_t<aux::is_base_of<internal_event, TEvent>::value,
        aux::type_list<TEvent>,
        aux::type_list<>>;
};
template <class T, class TEvent> struct get_sub_internal_events_impl<sm<T>, TEvent> {
...
template <class... Ts> using get_events = aux::type_list<typename Ts::event...>;
template <class T> struct get_exception : aux::type_list<> {};
template <class T> struct get_exception<exception<T>> : aux::type_list<exception<T>> {};
template <class... Ts> using get_exceptions = aux::join_t<typename get_exception<Ts>::type...>;
template <class... Ts>
using get_states = aux::join_t<aux::type_list<typename Ts::src_state, typename Ts::dst_state>...>;
template <class... Ts>
using get_initial_states = aux::join_t<typename aux::conditional<Ts::initial,
    aux::type_list<typename Ts::src_state>,
    aux::type_list<>>::type...>;
template <class... Ts>
using get_history_states = aux::join_t<typename aux::conditional<!Ts::history && Ts::initial,
    aux::type_list<typename Ts::src_state>,
    aux::type_list<>>::type...>;
template <class> struct get_sub_sm : aux::type_list<> {};
template <class T>
struct get_sub_sm<sm<T>> : aux::join_t<aux::type_list<T>, typename sm<T>::state_machines> {};
template <class... Ts> using get_sub_sms = aux::join_t<typename get_sub_sm<Ts>::type...>;
template <class... Ts> using get_sm_t    = aux::type_list<typename Ts::sm...>;
template <class... Ts>
using get_non_empty_t                   = aux::join_t<typename aux::
                          conditional<aux::is_empty<Ts>::value, aux::type_list<>, aux::type_list<Ts>>::type...>;
template <class... Ts> using merge_deps = aux::join_t<typename Ts::deps...>;
template <class> struct sub_sm;
template <class T> struct sub_sm<sm_impl<sm_policy<T>>> {
...
template <class T, class U> struct sub_sm<sm_impl<sm_policy<T, aux::identity<U>>>> {
    template <class... TPolicies>
    constexpr static sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>> &get(
        aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>> *object) {
        return static_cast<aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>> &>(
...
template <class T, class... TPolicies> struct rebind_impl {
...
template <class, class> struct convert_to_sm;
template <class T, class... Ts> struct convert_to_sm<T, aux::type_list<Ts...>> {
...
template <class> class sm;
template <class> struct sm_impl;
template <class...> struct transitions;
template <class...> struct transitions_sub;
template <class T, class... Ts> struct transitions<T, Ts...> {
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute(const TEvent &event,
        SM                                     &sm,
        TDeps                                  &deps,
        TSubs                                  &subs,
...
template <class T> struct transitions<T> {
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute(const TEvent &event,
        SM                                     &sm,
        TDeps                                  &deps,
...
template <> struct transitions<aux::true_type> {
    template <class TEvent, class SM, class TDeps, class TSubs>
    static bool execute(const TEvent &event,
        SM                           &sm,
        TDeps                        &deps,
        TSubs                        &subs,
        typename SM::state_t         &current_state) {
...
template <> struct transitions<aux::false_type> {
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute(const TEvent &, SM &, TDeps &, TSubs &, typename SM::state_t &) {
        return false;
    }
};
template <class Tsm, class T, class... Ts> struct transitions_sub<sm<Tsm>, T, Ts...> {
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute(const TEvent &event,
        SM                                     &sm,
        TDeps                                  &deps,
    ...
    constexpr static bool execute(const anonymous &event,
        SM                                        &sm,
        TDeps                                     &deps,
        TSubs                                     &subs,
        typename SM::state_t                      &current_state) {
    ...
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute_impl(const TEvent &event,
        SM                                          &sm,
        TDeps                                       &deps,
        TSubs                                       &subs,
        typename SM::state_t                        &current_state) {
...
template <class Tsm> struct transitions_sub<sm<Tsm>> {
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool
    execute(const TEvent &event, SM &, TDeps &deps, TSubs &subs, typename SM::state_t &) {
        return sub_sm<sm_impl<Tsm>>::get(&subs).process_event(event, deps, subs);
    }
    template <class, class SM, class TDeps, class TSubs>
    constexpr static bool
    execute(const anonymous &, SM &, TDeps &, TSubs &, typename SM::state_t &) {
        return false;
    }
};
} // namespace back
namespace back {
template <class> class sm;
template <class> struct state;
template <class> struct event;
template <class...> struct transitions;
template <class...> struct transitions_sub;
template <class, class> struct state_mappings;
template <class S, class... Ts> struct state_mappings<S, aux::type_list<Ts...>> {
    using element_type = state<S>;
    using types        = aux::type_list<Ts...>;
};
template <class, class> struct event_mappings;
template <class E, class... Ts> struct event_mappings<E, aux::inherit<Ts...>> {
    using element_type = event<E>;
    using types        = aux::type_list<Ts...>;
};
template <class...> struct unique_mappings;
template <class, class...> struct unique_mappings_impl;
template <class... Ts> using unique_mappings_t = typename unique_mappings<Ts...>::type;
template <class, class, class, class R> struct get_mapping : aux::type_list<R> {};
template <class E, class T, class R>
struct get_mapping<event<E>, event<E>, T, R>
    : aux::type_list<event_mappings<E,
...
template <bool, class, class...> struct conditional_mapping;
...
struct mappings<aux::pool<Ts...>>
    : unique_mappings_t<event_mappings<typename Ts::event,
          aux::inherit<state_mappings<typename Ts::src_state, aux::type_list<Ts>>>>...> {};
template <class T> using mappings_t = typename mappings<T>::type;
template <class, class TUnexpected> transitions<TUnexpected> get_state_mapping_impl(...);
template <class T, class, class... Ts>
transitions<Ts...> get_state_mapping_impl(state_mappings<T, aux::type_list<Ts...>> *);
template <class T, class TMappings, class TUnexpected> struct get_state_mapping {
...
template <class S> transitions_sub<S> get_sub_state_mapping_impl(...);
template <class T, class... Ts>
    ...
    struct type : sub_sm_mapping {
        ...
        constexpr static bool execute(const TEvent &event,
            ...
            return sub_sm_mapping::template execute<TEvent, TSm, TDeps, TSubs>(
                   ...
                   fallback_event_mapping::template execute<TEvent, TSm, TDeps, TSubs>(
                       event, sm, deps, subs, current_state);
        }
    };
};
...
template <class> transitions<aux::true_type> get_event_mapping_impl(...);
...
namespace policies {
    ...
    template <class T> struct dispatch : aux::pair<dispatch_policy__, T> {
        using type = T;
    };
    struct jump_table {
        template <int,
        ...
        static bool dispatch(sm_impl &self,
            ...
            constexpr static dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(
                sizeof...(TStates))] = {
    ...
    struct branch_stm {
        ...
        template <int N,
            ...
            class TState,
            class... TStates>
        constexpr static bool dispatch(sm_impl &self,
            State                              &current_state,
            const TEvent                       &event,
            TDeps                              &deps,
            TSubs                              &subs,
    ...
    struct switch_stm {
        ...
        constexpr static bool dispatch(sm_impl &self,
            ...
            case N:
                return get_state_mapping_t<TState,
                    TMappings,
                    typename sm_impl::has_unexpected_events>::
                    template execute<TEvent, sm_impl, TDeps, TSubs>(
                        event, self, deps, subs, current_state);
    ...
    struct fold_expr {
        template <class TMappings,
            ...
            class TSubs,
            class... TStates>
        constexpr static bool dispatch_impl(sm_impl &self,
            State                                   &current_state,
            ...
            TSubs        &subs,
            const aux::type_list<TStates...> &) {
            return ((current_state == Ns ? get_state_mapping_t<TStates,
                                               TMappings,
                                               typename sm_impl::has_unexpected_events>::
                                               template execute<TEvent, sm_impl, TDeps, TSubs>(
                                                   event, self, deps, subs, current_state)
...
namespace policies {
    ...
    template <class T> struct logger : aux::pair<logger_policy__, logger<T>> {
        using type = T;
    };
    template <class> struct get_state_name;
    template <class T> struct get_state_name<aux::string<T>> {
        using type = aux::string<T>;
    };
    template <class T, class... Ts> struct get_state_name<aux::string<sm<sm_policy<T, Ts...>>>> {
        using type = typename get_state_name<aux::string<sm_policy<T, Ts...>>>::type;
    };
    template <class T> struct get_state_name<aux::string<sm_policy<T>>> {
        using type = aux::string<T>;
    };
    template <class T, class TName> struct get_state_name<aux::string<sm_policy<T, TName>>> {
        using type = aux::string<T(typename TName::type)>;
    };
    template <class T> using get_state_name_t = typename get_state_name<T>::type;
    ...
    constexpr void log_process_event(const aux::type<TLogger> &, TDeps &deps, const TEvent &event) {
        return static_cast<aux::pool_type<TLogger &> &>(deps).value.template log_process_event<SM>(
            event);
    }
    template <class, class TDeps, class TSrcState, class TDstState>
    constexpr void
    log_state_change(const aux::type<no_policy> &, TDeps &, const TSrcState &, const TDstState &) {}
    template <class SM, class TLogger, class TDeps, class TSrcState, class TDstState>
    constexpr void log_state_change(const aux::type<TLogger> &,
        TDeps &deps,
        const TSrcState &,
        const TDstState &) {
    ...
    constexpr void log_action(const aux::type<TLogger> &,
        TDeps         &deps,
        const TAction &action,
        const TEvent  &event) {
        return static_cast<aux::pool_type<TLogger &> &>(deps).value.template log_action<SM>(
            action, event);
    }
    template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
    constexpr void log_action(const aux::type<TLogger> &,
    ...
    constexpr void
    log_guard(const aux::type<no_policy> &, TDeps &, const TGuard &, const TEvent &, bool) {}
    template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
    constexpr void log_guard(const aux::type<TLogger> &,
        TDeps        &deps,
        const TGuard &guard,
        const TEvent &event,
        bool          result) {
...
struct should_not_subclass_statemachine_class
    : integral_constant<bool,
          is_empty<typename Tsm::sm>::value ||
...
struct no_policy : policies::thread_safety_policy__ {
    ...
    template <class> using rebind   = no_policy;
    template <class...> using defer = no_policy;
    using const_iterator            = no_policy;
    using flag                      = no_policy;
    __BOOST_SML_ZERO_SIZE_ARRAY(aux::byte);
};
template <class TDefault, class> TDefault        get_policy(...);
template <class, class T, class TPolicy> TPolicy get_policy(aux::pair<T, TPolicy> *);
template <class SM, class... TPolicies> struct sm_policy {
    ...
    using process_queue_policy = decltype(get_policy<no_policy, policies::process_queue_policy__>(
        (aux::inherit<TPolicies...> *) 0));
    using logger_policy        = decltype(get_policy<no_policy, policies::logger_policy__>(
        (aux::inherit<TPolicies...> *) 0));
    using testing_policy       = decltype(get_policy<no_policy, policies::testing_policy__>(
        (aux::inherit<TPolicies...> *) 0));
    ...
    using dispatch_policy =
        decltype(get_policy<default_dispatch_policy, policies::dispatch_policy__>(
            (aux::inherit<TPolicies...> *) 0));
    template <class T> using rebind = typename rebind_impl<T, TPolicies...>::type;
};
} // namespace back
namespace concepts {
...
template <class> aux::true_type test_callable(...);
template <class, class T>
struct callable
    : decltype(test_callable<
          aux::inherit<aux::conditional_t<__is_class(T), T, aux::none_type>, callable_fallback>>(
          0)) {};
...
namespace concepts {
template <class T> decltype(aux::declval<T>().operator()()) composable_impl(int);
template <class> void                                       composable_impl(...);
template <class T> struct composable : aux::is<aux::pool, decltype(composable_impl<T>(0))> {};
} // namespace concepts
...
struct sm_impl : aux::conditional_t<aux::should_not_subclass_statemachine_class<Tsm>::value,
    ...
    using has_history_states   = aux::integral_constant<bool,
          aux::size<initial_states_t>::value != aux::size<history_states_t>::value>;
    using sub_internal_events_t =
        aux::apply_t<aux::unique_t, aux::apply_t<get_sub_internal_events, transitions_t>>;
    using events_t     = aux::apply_t<aux::unique_t,
    ...
    constexpr bool process_event(const TEvent &event, TDeps &deps, TSubs &subs) {
        ...
        do {
            do {
                ...
                deps, subs, changed, aux::type<defer_queue_t<TEvent>>{}, events_t{}));
        } while (process_queued_events(
            deps, subs, queued_handled, aux::type<process_queue_t<TEvent>>{}, events_t{}));
        return handled && queued_handled;
    }
    constexpr void                         initialize(const aux::type_list<> &) {}
    template <class TState> constexpr void initialize(const aux::type_list<TState> &) {
        current_state_[0] = aux::get_id<state_t, TState>((states_ids_t *) 0);
    }
    template <class... TStates> constexpr void initialize(const aux::type_list<TStates...> &) {
        auto region = 0;
#if defined(__cpp_fold_expressions)
        ((current_state_[region++] = aux::get_id<state_t, TStates>((states_ids_t *) 0)), ...);
#else
        (void) aux::swallow{0,
            (current_state_[region++] = aux::get_id<state_t, TStates>((states_ids_t *) 0), 0)...};
#endif
    }
    template <class TDeps, class TSubs> constexpr void start(TDeps &deps, TSubs &subs) {
    ...
    constexpr bool process_event_impl(const TEvent &event,
        TDeps                                      &deps,
        TSubs                                      &subs,
        const aux::type_list<TStates...>           &states,
    ...
    constexpr bool
    process_event_except_impl(const TEvent &event, TDeps &deps, TSubs &subs, aux::false_type) {
        return process_event_impl<TMappings>(
            event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
    }
    template <class TMappings, class TEvent, class TDeps, class TSubs>
    constexpr bool process_event_except_impl(const TEvent &event,
        TDeps                                             &deps,
        TSubs                                             &subs,
        state_t                                           &current_state,
        aux::false_type) {
    ...
    template <class TMappings, class TEvent, class TDeps, class TSubs>
    bool process_event_except_impl(const TEvent &event,
        TDeps                                   &deps,
        TSubs                                   &subs,
        state_t                                 &current_state,
        ...
        try {
            return process_event_impl<TMappings>(event, deps, subs, states_t{}, current_state);
        } catch (...) { return process_exception(deps, subs, exceptions{}); }
    }
    template <class TMappings, class TEvent, class TDeps, class TSubs>
    ...
    bool process_exception(TDeps &deps, TSubs &subs, const aux::type_list<E, Es...> &) {
        ...
        } catch (...) { return process_exception(deps, subs, aux::type_list<Es...>{}); }
    }
#endif
    template <class TDeps, class TSubs, class... TEvents>
    constexpr bool process_defer_events(TDeps &,
        TSubs &,
        const bool,
    ...
    template <class TDeps, class TSubs, class... TEvents>
    constexpr bool process_queued_events(TDeps &,
        TSubs &,
        bool &,
        const aux::type<no_policy> &,
        const aux::type_list<TEvents...> &) {
    ...
    template <int... Ns> constexpr bool is_terminated_impl(aux::index_sequence<Ns...>) const {
#if defined(__cpp_fold_expressions)
...
template <class Tsm> class sm {
  ...
  public:
    ...
    template <class T = aux::identity<sm_t>,
        class... TStates,
        __BOOST_SML_REQUIRES(
            !aux::is_same<no_policy, typename Tsm::testing_policy>::value && aux::always<T>::value)>
    ...
    template <class T> constexpr operator const T &() {
...
template <class TSubs> using get_root_sm_t = decltype(get_root_sm_impl((TSubs *) 0));
template <class, class> aux::type_list<action_base> args1__(...);
template <class T, class E> auto args1__(int) -> aux::function_traits_t<decltype(&T::operator())>;
template <class T, class E>
auto args__(int) -> aux::function_traits_t<
                     decltype(&T::__BOOST_SML_TEMPLATE_KEYWORD operator()<back::get_event_t<E>>)>;
template <class T, class E> auto args__(...) -> decltype(args1__<T, E>(0));
...
constexpr decltype(auto)
get_arg(const aux::type<back::defer<TEvents...>> &, const TEvent, Tsm &sm, TDeps &) {
...
template <class, class, class> struct call;
template <class TEvent> struct call<TEvent, aux::type_list<>, back::no_policy> {
    template <class T, class Tsm, class TDeps, class TSubs>
    constexpr static auto execute(T object, const TEvent &, Tsm &, TDeps &, TSubs &) {
        return object();
    }
};
template <class TEvent, class TLogger> struct call<TEvent, aux::type_list<>, TLogger> {
    template <class T, class Tsm, class TDeps, class TSubs>
    ...
    execute_impl(const aux::type<bool> &, T object, const TEvent &event, TDeps &deps) {
    ...
    template <class Tsm, class T, class TDeps>
    constexpr static auto
    execute_impl(const aux::type<void> &, T object, const TEvent &event, TDeps &deps) {
        back::policies::log_action<Tsm>(aux::type<TLogger>{}, deps, object, event);
        object();
    }
};
template <class TEvent> struct call<TEvent, aux::type_list<TEvent>, back::no_policy> {
    template <class T, class Tsm, class TDeps, class TSubs>
    constexpr static auto execute(T object, const TEvent &event, Tsm &, TDeps &, TSubs &) {
        return object(event);
    }
};
template <class TEvent, class TLogger> struct call<TEvent, aux::type_list<TEvent>, TLogger> {
    template <class T, class Tsm, class TDeps, class TSubs>
    constexpr static auto execute(T object, const TEvent &event, Tsm &, TDeps &deps, TSubs &) {
...
template <class TEvent> struct call<TEvent, aux::type_list<action_base>, back::no_policy> {
    template <class T, class Tsm, class TDeps, class TSubs>
    static auto execute(T object, const TEvent &event, Tsm &sm, TDeps &deps, TSubs &subs) {
        return object(event, sm, deps, subs);
    }
};
template <class TEvent, class TLogger> struct call<TEvent, aux::type_list<action_base>, TLogger> {
    template <class T, class Tsm, class TDeps, class TSubs>
    constexpr static auto
    execute(T object, const TEvent &event, Tsm &sm, TDeps &deps, TSubs &subs) {
        return object(event, sm, deps, subs);
    }
};
template <class TEvent, class... Ts> struct call<TEvent, aux::type_list<Ts...>, back::no_policy> {
    template <class T, class Tsm, class TDeps, class TSubs>
...
template <class... Ts> class seq_ : operator_base {
  public:
  ...
  private:
    ...
    constexpr void for_all(const aux::index_sequence<Ns...> &,
        const TEvent &event,
        Tsm          &sm,
        TDeps        &deps,
        TSubs        &subs) {
...
template <class... Ts> class and_ : operator_base {
  public:
  ...
  private:
    template <int... Ns, class TEvent, class Tsm, class TDeps, class TSubs>
    constexpr auto for_all(const aux::index_sequence<Ns...> &,
        const TEvent &event,
        Tsm          &sm,
        TDeps        &deps,
        TSubs        &subs) {
...
template <class... Ts> class or_ : operator_base {
  public:
...
template <class T> class not_ : operator_base {
  public:
...
template <class T> using logger                            = back::policies::logger<T>;
template <class T> using thread_safe                       = back::policies::thread_safe<T>;
template <class T> using dispatch                          = back::policies::dispatch<T>;
template <template <class...> class T> using defer_queue   = back::policies::defer_queue<T>;
template <template <class...> class T> using process_queue = back::policies::process_queue<T>;
...
#else
template <class T, class... TPolicies> using sm = back::sm<back::sm_policy<T, TPolicies...>>;
#endif
#if defined(__cpp_deduction_guides)
namespace front {
template <class T, class... TPolicies> struct sm : back::sm<back::sm_policy<T, TPolicies...>> {
...
auto transitional_impl(T &&t) -> aux::always<typename T::dst_state,
                                  ...
                                  decltype(T::initial),
                                  decltype(T::history)>;
template <class T> struct transitional : decltype(transitional_impl(aux::declval<T>())){};
} // namespace concepts
namespace front {
namespace actions {
    struct process {
        template <class TEvent> class process_impl : public action_base {
          public:
        ...
        template <class TEvent> constexpr auto operator()(const TEvent &event) {
...
template <class TEvent> struct event {
    template <class T, __BOOST_SML_REQUIRES(concepts::callable<bool, T>::value)>
...
template <class, class> struct transition_eg;
template <class> struct state;
template <class TState> struct state_impl {
...
template <class TState> struct state : state_impl<state<TState>> {
    using type                    = TState;
    ...
    template <class... Ts> constexpr auto operator()(const state<Ts> &...) const {
        return state<TState(Ts...)>{};
    }
    template <class T> constexpr auto operator=(const T &t) const {
        return transition<T, state>{t, *this};
    }
    template <class T> constexpr auto sm() const {
...
#else
template <class T, class = void> struct state_sm {
    using type = state<T>;
};
template <class T> struct state_sm<T, aux::enable_if_t<concepts::composable<T>::value>> {
...
namespace front {
struct internal {};
template <class, class> struct ignore;
template <class E, class... Ts> struct ignore<E, aux::type_list<Ts...>> {
    template <class T> struct non_events {
        using type =
            aux::conditional_t<aux::is_same<back::get_event_t<E>,
...
template <class...> struct transition;
template <class E, class G> struct transition<front::event<E>, G> {
    template <class T> constexpr auto operator/(const T &t) const {
...
template <class E, class G, class A> struct transition<front::event<E>, G, A> {
    front::event<E> e;
    G               g;
    A               a;
};
...
struct transition<state<S2>, G, A>
    ...
    template <class T> constexpr auto operator=(const T &) const {
...
struct transition_sg<state<S2>, G>
    ...
    transition_sg(const state<S2> &, const G &g)
        : transition<state<internal>, state<S2>, front::event<back::anonymous>, G, none>{g,
              none{}} {}
    template <class T> constexpr auto operator/(const T &t) const {
...
template <class S2, class E> struct transition<state<S2>, front::event<E>> {
    template <class T> constexpr auto operator=(const T &) const {
...
template <class E, class G> struct transition_eg<front::event<E>, G> {
    template <class T> constexpr auto operator/(const T &t) const {
        return transition<front::event<E>, G, aux::zero_wrapper<T>>{e, g, aux::zero_wrapper<T>{t}};
    }
    front::event<E> e;
...
template <class E, class A> struct transition_ea<front::event<E>, A> {
    front::event<E> e;
    A               a;
};
...
struct transition<state<S1>, transition_sa<state<S2>, A>>
    ...
    constexpr transition(const state<S1> &, const transition_sa<state<S2>, A> &t)
        : transition<state<S1>, state<S2>, front::event<back::anonymous>, always, A>{always{},
              t.a} {}
};
...
constexpr void
update_composite_states(TSubs &subs, aux::true_type, const aux::type_list<THs...> &) {
...
constexpr void update_current_state(SM &,
    TDeps &deps,
    TSubs &,
    typename SM::state_t       &current_state,
...
struct transition<state<S1>, state<S2>, front::event<E>, G, A> {
    ...
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool execute(const TEvent &event,
        SM                              &sm,
        TDeps                           &deps,
        TSubs                           &subs,
        typename SM::state_t            &current_state,
        ...
        if (call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(
                g, event, sm, deps, subs)) {
            sm.process_internal_event(
                back::on_exit<back::_, TEvent>{event}, deps, subs, current_state);
            update_current_state(sm,
                deps,
                subs,
                current_state,
            ...
            sm.process_internal_event(
                back::on_entry<back::_, TEvent>{event}, deps, subs, current_state);
            return true;
        }
        return false;
    }
...
struct transition<state<S1>, state<S2>, front::event<E>, always, none> {
    ...
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool execute(const TEvent &,
        SM                   &sm,
        TDeps                &deps,
        TSubs                &subs,
        typename SM::state_t &current_state,
...
struct transition<state<internal>, state<S2>, front::event<E>, always, none> {
    ...
    template <class TEvent, class SM, class TDeps, class TSubs, class... Ts>
    constexpr bool
    execute(const TEvent &, SM &, TDeps &, TSubs &, typename SM::state_t &, Ts &&...) {
        return true;
    }
    __BOOST_SML_ZERO_SIZE_ARRAY(aux::byte);
};
...
using _ = back::_;
#if !(defined(_MSC_VER) && !defined(__clang__))
template <class TEvent> constexpr front::event<TEvent> event{};
#else
template <class TEvent> front::event<TEvent> event __BOOST_SML_VT_INIT;
#endif
template <class TEvent>
...
template <class T> front::event<back::exception<T>> exception __BOOST_SML_VT_INIT;
using anonymous = back::anonymous;
using initial   = back::initial;
#if !(defined(_MSC_VER) && !defined(__clang__))
template <class T> constexpr typename front::state_sm<T>::type state{};
#else
template <class T> typename front::state_sm<T>::type state __BOOST_SML_VT_INIT;
#endif
#if !(defined(__ICCARM__))
inline namespace literals {
#if defined(__cpp_nontype_template_parameter_class) ||                                             \
    defined(__cpp_nontype_template_args) && __cpp_nontype_template_args >= 201911L
template <aux::fixed_string Str> constexpr auto operator""_s() {
    return []<auto... Ns>(aux::index_sequence<Ns...>) {
        return front::state<aux::string<char, Str.data[Ns]...>>{};
    }(aux::make_index_sequence<Str.size>{});
}
template <aux::fixed_string Str> constexpr auto operator""_e() {
    return []<auto... Ns>(aux::index_sequence<Ns...>) {
        return event<aux::string<char, Str.data[Ns]...>>;
    }(aux::make_index_sequence<Str.size>{});
}
#elif !(defined(_MSC_VER) && !defined(__clang__))
template <class T, T... Chrs> constexpr auto operator""_s() {
    return front::state<aux::string<T, Chrs...>>{};
}
template <class T, T... Chrs> constexpr auto operator""_e() {
