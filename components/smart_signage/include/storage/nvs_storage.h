...
class NvsStorage : public IStorage {
  public:
    ...
    // ───────── Scalars ─────────
    bool writeU32(const Key &key, uint32_t value) override {
        nvs_handle_t h;
    ...
    bool readU32(const Key &key, uint32_t &value) override {
        nvs_handle_t h;
    ...
    bool readU32OrDefault(const Key &key, uint32_t &value, uint32_t defValue) override {
        if (readU32(key, value)) return true;
        value = defValue;
        // Try to persist the default so future reads succeed
        (void) writeU32(key, defValue);
        return true;
    }

    // ───────── Blobs ─────────
    bool writeBlob(const Key &key, const void *data, size_t len) override {
        nvs_handle_t h;
        if (nvs_open(ns_.c_str(), NVS_READWRITE, &h) != ESP_OK) return false;
    ...
    bool readBlob(const Key &key, void *data, size_t &inout_len) override {
        nvs_handle_t h;
        if (nvs_open(ns_.c_str(), NVS_READONLY, &h) != ESP_OK) return false;
    ...
    // ───────── Strings ─────────
    bool writeString(const Key &key, const char *str) override {
        nvs_handle_t h;
        if (nvs_open(ns_.c_str(), NVS_READWRITE, &h) != ESP_OK) return false;
    ...
    bool readString(const Key &key, char *buf, size_t &inout_len) override {
        nvs_handle_t h;
        if (nvs_open(ns_.c_str(), NVS_READONLY, &h) != ESP_OK) return false;
