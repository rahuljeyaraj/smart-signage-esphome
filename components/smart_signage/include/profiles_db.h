...
class ProfilesDB {
  ...
  private:
    ...
    static uint16_t to_ms_(JsonVariantConst gap_ms, JsonVariantConst repeat_s) {
        ...
        if (repeat_s.is<uint32_t>()) return static_cast<uint16_t>(repeat_s.as<uint32_t>() * 1000u);
        if (repeat_s.is<float>()) return static_cast<uint16_t>(repeat_s.as<float>() * 1000.0f);
        return 0;
    }
    void map_audio_(JsonVariantConst node, AudioBinding &a) const {
        ...
        if (node.is<JsonObject>()) {
            ...
            a.spec.loopCount      = node["loops"].is<uint32_t>()
    ...
    void map_led_(JsonVariantConst node, LedBinding &lb) const {
        ...
        if (strcmp(eff, "off") == 0) {
            ...
            lb.u.blink.on_ms  = node["on_ms"].is<uint32_t>() ? node["on_ms"].as<uint32_t>() : 120;
            lb.u.blink.off_ms = node["off_ms"].is<uint32_t>() ? node["off_ms"].as<uint32_t>() : 120;
            lb.u.blink.count  = node["count"].is<uint32_t>() ? node["count"].as<uint32_t>() : 3;
            return;
